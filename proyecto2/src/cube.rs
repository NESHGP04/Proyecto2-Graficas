...
impl Cube {
    ...
    pub fn hit(&self, ray: &Ray, t_min: f32, t_max: f32) -> Option<HitRecord> {
        ...
        if t_near >= t_min && t_near <= t_max {
            ...
            let material_color = self.material.get_color(u, v);
...
// Ground plane with material support
#[derive(Debug, Clone)]
pub struct Plane {
    pub point: Vector3,
    pub normal: Vector3,
...
impl Plane {
    ...
    pub fn hit(&self, ray: &Ray, t_min: f32, t_max: f32) -> Option<HitRecord> {
        ...
        if t >= t_min && t <= t_max {
            ...
            // Calculate UV for plane (simple tiling)
            let u = (hit_point.x * 0.2).fract();
            let v = (hit_point.z * 0.2).fract();
            
            let material_color = self.material.get_color(u, v);
            let mut hit_record = HitRecord::new(hit_point, self.normal, t, material_color);
            hit_record.set_face_normal(ray, self.normal);
            hit_record.material = Some(self.material.clone());
